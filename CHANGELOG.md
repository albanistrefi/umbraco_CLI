...
- `api --dry-run` previews list the complete header set the request will carry: implicit `Authorization: Bearer <redacted>`, `User-Agent`, and `Content-Type` (`application/json` for `--body`, `multipart/form-data; boundary=<generated when sent>` for `--form`), with `--header` values overriding
