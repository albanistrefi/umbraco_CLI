...
- fixed `--merge-json` replacing the whole `variants` array instead of merging it. The deep merge matches array entries by identity, but it only recognized an identity built on `alias` — and variant objects carry no alias, only `culture`/`segment` — so `--merge-json '{"variants":[{"culture":"de-DE","name":"X"}]}'` sent a payload containing that one variant and nothing else: renaming one culture dropped the others. The payload's own `variants` array is now keyed by culture + segment, so a patch naming one culture updates only that variant and leaves the rest of the payload alone. The new identity is scoped to that one field and nothing else: alias-keyed arrays (`values`, a doctype's `properties`) keep the alias + culture + segment key they already had, an array whose entries carry none of the three (a doctype's `containers`, keyed by an id the caller does not restate) still replaces wholesale, as does a variants patch that names no culture, and anything reached by recursion — a property value that happens to be an array of culture-tagged objects, a data type setting, a nested `variants` field inside a block payload — is still replaced wholesale rather than merged entry-wise. `--json` is unchanged and still replaces the payload wholesale — that remains the way to say "these variants and no others". Covers `document update --merge-json`, the `--ids`/`--from-file` batch and `document update-bulk` paths, and `document create --from-blueprint --json`. Verified live on 18.1 against a disposable two-culture document, old binary against new: on `document update` the wire payload really did carry only the patched variant, though the damage was invisible because the server leaves cultures absent from a PUT untouched; on `document create --from-blueprint --json` there is no existing document to fall back on, and the old binary created a document holding **only** the patched de-DE variant with the default en-US culture missing entirely, while the fixed binary created both cultures with the untouched one keeping its blueprint name and both culture-scoped values intact
