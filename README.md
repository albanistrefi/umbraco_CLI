...
operations — 290 commands with a scriptable exit-code contract.
...
- `doctype` (17) / `mediatype` (10) / `membertype` (10) — the full schema type family, all with `--recursive --types-only` folder handling and `create-folder`/`delete-folder`
...
Total: **290 runnable commands** counting every nested subcommand. Group counts above are direct subcommands; nested subgroups like `document version`, `document bin`, and the `automate` subgroups add the rest.
