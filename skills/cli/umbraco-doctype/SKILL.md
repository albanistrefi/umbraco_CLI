...
Reads a file written by any 'doctype … --backup' command and PUTs the saved document type back to the server, then re-reads it and fails if a saved property is missing afterwards. The endpoint is derived from the id inside the envelope (the file cannot steer the write elsewhere). This restores the entity's fields and values; it does not undo a move, publish state, or delete.
