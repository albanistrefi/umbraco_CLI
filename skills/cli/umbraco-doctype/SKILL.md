...
POST /document-type/folder. Creates a folder in the document type tree; --parent nests it inside an existing folder. Put a type inside it with 'umbraco doctype create --json '{..., "parent": {"id": "<folder id>"}}'' or 'umbraco doctype move <id> --to <folder id>'. After the create the folder is read back, so the result is the persisted record.
